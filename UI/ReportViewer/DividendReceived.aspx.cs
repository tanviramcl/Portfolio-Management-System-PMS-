...
public partial class UI_DividendReceived : System.Web.UI.Page
{
    ...
    protected void fundNameDropDownList_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
             taxRate = Convert.ToDecimal(TaxRateTextBox.Text.Trim());           
        }
        GrirdViewBookCloserDeatils(taxRate);
    }
    ...
    protected void postedButton_Click(object sender, EventArgs e)
    {
        ...
        foreach (DataGridItem gridRow in leftDataGrid.Items)
        {
            ...
            if (leftCheckBox.Checked)
            {
                ...
                double Tax_amt = Gross_amt * (tax_rate/100);
                ...
                accReceivedVCHNo = SaveDividendAccountVoucher(fund_code, totalShare, net_amt, Tax_amt, comp_name, diviRatePerShare, fy, DIV_RRCIVED_ACC_VCH_NO, LoginName);
                string accFracReceivedVCHNo = "";
                if (RECEIVABLE_excessSortage>0)
                 accFracReceivedVCHNo = SaveFractonDividendAccountVoucher(fund_code, totalShare,Convert.ToDouble( RECEIVABLE_excessSortage), comp_name, diviRatePerShare, fy, DIV_RRCIVED_ACC_VCH_NO, LoginName);
    ...
    public string SaveDividendAccountVoucher(int fund_code, long totalShare , double totalDivAmt, double taxAmount, string comp_name, decimal diviRate, string fy, string divReceivable_acc_vch_no, string LoginName)
    {
        ...
        try
        {
           ...
           // string accountSchema = "UNIT_BR";
            ...
            htInsert.Add("TRAN_DATE", DateTime.Today.ToString("dd-MMM-yyyy"));
    ...
    public string SaveFractonDividendAccountVoucher(int fund_code, long totalShare, double totalFracDivAmt, string comp_name, decimal diviRate, string fy, string divReceivable_acc_vch_no, string LoginName)
    {
    ...
    public void GrirdViewBookCloserDeatils( decimal taxRate)
    {
        DataTable bookCloser_details = new DataTable();

        StringBuilder sbMst = new StringBuilder();
       
        sbMst.Append(" SELECT A.*,A.GROSS_DIVIDEND*ROUND("+ taxRate + "/100,2) AS TAX_AMT,A.GROSS_DIVIDEND-A.GROSS_DIVIDEND*ROUND(" + taxRate + "/100,2) AS NET_AMT,0 AS FRAC_DIV_AMT, B.FY,B.FY_PART,B.AGM,B.RECORD_DT,B.TYPE, C.F_NAME,D.COMP_NM FROM(SELECT * FROM BOOK_CL_DETAILS  WHERE PAYMENT_DATE IS NULL) A ");
